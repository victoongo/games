...
while running:
    ...
    if game_on:
        ...
        pygame.draw.rect(screen, paddle_color, (left_paddle_pos.x, left_paddle_pos.y, paddle_width, paddle_height))
        pygame.draw.rect(screen, paddle_color, (right_paddle_pos.x, right_paddle_pos.y, paddle_width, paddle_height))
    ...
    if keys[pygame.K_p] and game_on == False:
        game_on = True
        ball_pos = pygame.Vector2(screen.get_width() / 2, screen.get_height() / 2)
        ball_velocity = pygame.Vector2(random.choice([-1, 1])*random.randint(beginning_ball_speed[0], beginning_ball_speed[1]), \
                                       random.choice([-1, 1])*random.randint(beginning_ball_speed[0], beginning_ball_speed[1]))
